      ...
      <main className="flex-1 flex flex-col min-w-0">
        ...
        <div className="p-6 md:p-8 overflow-y-auto">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="bg-white p-5 rounded-2xl border border-borderLight shadow-sm flex flex-col h-[400px]">
              <div className="flex justify-between items-center mb-2 border-b border-gray-100 pb-2">
                ...
                <span className="text-[9px] font-bold text-primary bg-primaryLight px-2 py-0.5 rounded-full uppercase tracking-wide">Surplus surplus-surplus</span>
              </div>
              ...
              <div className="flex-1 overflow-y-auto space-y-3 mb-3 pr-1 text-xs no-scrollbar">
                {chatHistory.map((msg, index) => (
                  <div key={index} className={`flex flex-col ${msg.role === 'user' ? 'items-end' : 'items-start'}`}>
                    ...
                    {/* Render Collapsible Reasoning Process and Advice for AI responses */}
                      ...
                      <div className="mt-2 w-full space-y-2">
                        ...
                        <div className="border border-gray-200 rounded-lg overflow-hidden bg-gray-50/50">
                          <button
                            ...
                            className="w-full px-3 py-1.5 flex justify-between items-center text-[9px] font-black text-textMuted uppercase hover:bg-gray-100 transition-colors"
                          >
                          ...
                          {openReasoningIndex === index && (
                            <div className="p-3 border-t border-gray-150 text-[11px] space-y-2 bg-white text-textMuted font-semibold animate-fadeIn">
                              <div>
                                <span className="text-[8px] font-extrabold text-gray-400 uppercase tracking-wider block mb-0.5">Classified Intent</span>
                              ...
                              <div>
                                <span className="text-[8px] font-extrabold text-gray-400 uppercase tracking-wider block mb-0.5">Fetched Account Data</span>
                              ...
                              <div>
                                <span className="text-[8px] font-extrabold text-gray-400 uppercase tracking-wider block mb-0.5">Calculations & Formulas</span>
                                <div className="space-y-1 text-[9px] font-mono text-darkNavy">
                                  {msg.structuredResponse.intent === 'affordability' && (
                                    <>
                                      <div>Price: ₹{msg.structuredResponse.calculations.targetAmount?.toLocaleString()}</div>
                                      <div>Affordable: {msg.structuredResponse.calculations.affordable ? 'Yes' : 'No'}</div>
                                      <div>Months needed: {msg.structuredResponse.calculations.monthsNeeded}</div>
                                      <div className="text-[8px] text-textMuted italic mt-0.5">Formula: Price / Net Savings</div>
                                    </>
                                  )}
                                  {msg.structuredResponse.intent === 'savings_rate' && (
                                    <>
                                      <div>Savings Rate: {msg.structuredResponse.calculations.savingsRate}%</div>
                                      <div>Rating: {msg.structuredResponse.calculations.rating}</div>
                                      <div className="text-[8px] text-textMuted italic mt-0.5">Formula: (Net Savings / Income) * 100</div>
                                    </>
                                  )}
                                  {msg.structuredResponse.intent === 'budget_status' && (
                                    <>
                                      <div>Over budget count: {msg.structuredResponse.calculations.overBudgetCategories}</div>
                                      <div className="text-[8px] text-textMuted italic mt-0.5">Formula: Category Spent vs Split Limit</div>
                                    </>
                                  )}
                                  {msg.structuredResponse.intent === 'debt_load' && (
                                    <>
                                      <div>EMI to Income: {msg.structuredResponse.calculations.emiRatio}%</div>
                                      <div>Outstanding debt: ₹{msg.structuredResponse.calculations.totalOutstandingDebt?.toLocaleString()}</div>
                                      <div className="text-[8px] text-textMuted italic mt-0.5">Formula: (EMIs / Income) * 100</div>
                                    </>
                                  )}
                                </div>
                              </div>
                        ...
                        {msg.structuredResponse.structuredAdvice && msg.structuredResponse.structuredAdvice.length > 0 && (
                          <div className="space-y-1.5 w-full">
                            {msg.structuredResponse.structuredAdvice.map((adv, idx) => (
                              <div
                                key={idx}
                                className={`px-2.5 py-2 rounded-xl text-[10px] font-bold border flex items-start gap-1.5 leading-relaxed ${
                                  adv.type === 'success'
              ...
              <div className="flex gap-1.5 overflow-x-auto pb-2 no-scrollbar scroll-smooth">
                {suggestionChips.map((chip, idx) => (
                  <button
                    ...
                    className="px-2.5 py-1 bg-gray-50 border border-gray-200 hover:border-primary/20 hover:text-primary transition-all rounded-full whitespace-nowrap text-[9px] text-textMuted font-black uppercase shrink-0"
                  >
                    {chip}
                  </button>
                ))}
              ...
              {/* Chat Input Form */}
              <form onSubmit={handleSendChat} className="flex gap-2 border-t border-gray-100 pt-2 flex-shrink-0">
                <input
                  type="text"
                  placeholder="Ask Finance Fox AI..."
                  value={chatInput}
                  onChange={e => setChatInput(e.target.value)}
                  className="flex-1 bg-gray-50 border border-gray-200 rounded-xl px-3 py-2 text-xs focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all font-medium text-darkNavy"
                />
                <button
                  type="submit"
            ...
            <div className="bg-white p-5 rounded-2xl border border-borderLight shadow-sm flex flex-col h-[400px]">
              ...
              <div className="bg-darkNavy rounded-xl p-4 flex justify-between text-white mt-auto">
                <div>
            ...
            {/* EMI Tracker */}
            <div className="bg-white p-5 rounded-2xl border-2 border-blue-500 shadow-sm relative overflow-hidden flex flex-col h-[400px]">
              <div className="flex justify-between items-center mb-4">
